...
class MRRCascade {
  public:
    ...
    /// Synthesises N = ceil(n) IDENTICAL rings of order n/N, as in Sec. 3.B
    /// of [LIU25] (1.44 = 0.72+0.72, 2.1 = 0.7+0.7+0.7). Sec. 2 also allows
    /// 1+0.44 or 1+1+0.1, with unequal stages; we do not: every ring here is
    /// the same device.
    ...
    /// Cascade perturbed ring by ring: one draw per stage. The aggregate
    /// getters report the mean over the stages.
    static MRRCascade perturbed(const MRRCascade &nominal,
                                const std::vector<StageParams> &stage_params);

    /// Same parameters on every stage (R_custom <= 0 keeps the nominal radius).
    static MRRCascade perturbed(const MRRCascade &nominal, double r, double xi,
                                double n_eff, double n_g, double df,
                                double R_custom = -1.0);

    /// Order actually delivered: the sum over the stages, which once they
    /// differ is not N * order() of the first.
    ...
    /// Total response, H_tot(f) = PROD H_i(f).
    ...
    /**
     * @brief Band [Hz] over which |H_tot| tracks the ideal |2*pi*f|^n to
     *        within `tol_dB`.
     *
     * Measured, not estimated: with N stages each ring's relative deviation
     * multiplies, so the band narrows, but not by the cascaded-bandpass
     * formula. An amplitude width (|H|, 20*log10), the same convention
     * Simulation::Input::gaussian_matched() expects.
     */
