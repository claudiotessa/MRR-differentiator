...
MRR MRR::first_order(double R, double xi, double n_g) {
    if (!(xi > 0.0 && xi < 1.0))
    ...
    return MRR(R, xi, xi, n_g);
}

MRR MRR::fractional_order(double n, double R, double xi, double n_g) {
...
std::string MRR::params_string() const {
    ...
    std::snprintf(buf, sizeof(buf),
                  ...
                  usable_band() / 1e9, phase_transition_width() / 1e9);
